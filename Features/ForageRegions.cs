...
[Feature("Forage Regions")]
internal class ForageRegions
{
			...
			for (int i = r.Next(region.Min, region.Max); i > 0; i--)
			{
				for (int attempt = 0; attempt < 11; attempt++)
				{
					...
					Item forageItem = ItemQueryResolver.TryResolveRandomItem(
						forage, itemQueryContext, avoidRepeat: false, null, null, null, 
						(query, error) => 
						Monitor.Log($"Could not retrieve forage item '{query}' to spawn in region '{region.Id}' in '{__instance.Name}': {error}", LogLevel.Warn)
					);
					...
					{
						Monitor.Log($"Failed to spawn non-object forage item from entry '{forage.Id}' in region '{region.Id}' in '{__instance.Name}'.", LogLevel.Warn);
					}
					else
					{
